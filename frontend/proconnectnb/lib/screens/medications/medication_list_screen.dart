...
class _MedicationListScreenState extends State<MedicationListScreen> {
  ...
  Future<void> _createOrRemoveRappels({
    ...
    medicationProvider.toggleActive(med.id, value);

    final medicamentId = int.tryParse(med.id);
    if (medicamentId == null) return;

    await rappelProvider.deleteRappelByMedicamentId(medicamentId);
    if (!value) return;
  ...
  Future<void> _deleteSelected() async {
    ...
    for (final id in _selectedIds.toList()) {
      ...
      final success = await medicationProvider.deleteMedication(id);
