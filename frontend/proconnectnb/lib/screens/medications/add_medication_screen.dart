...
class _AddMedicationScreenState extends State<AddMedicationScreen> {
  ...
  Future<void> _createRappelsForMedication({
    ...
    await rappelProvider.deleteRappelByMedicamentId(medId);
    ...
    for (final schedule in schedules) {
      ...
      final rappel = Rappel(
        ...
        type: 'Médicament : $name',
        actif: true,
        medicamentId: medId,
        groupeId: 'med_$medId',
      );

      await rappelProvider.addRappel(rappel);
    ...
    try {
      ...
      if (_isEditMode) {
        success = await medicationProvider.updateMedication(
          ...
          urlPhoto: urlPhoto, aineId: widget.initialAineId, isActive: _isActive,
        );
        ...
        if (success && medId != null) {
          if (_isActive) {
            await _createRappelsForMedication(
              ...
              medId: medId, name: name, schedules: schedules,
            );
          } else {
            await rappelProvider.deleteRappelByMedicamentId(medId);
          }
        ...
        success = await medicationProvider.addMedication(
          name, marque, dosage, schedules,
          urlPhoto: urlPhoto, aineId: widget.initialAineId, isActive: _isActive,
        );

        if (success && _isActive) {
          final newMed = medicationProvider.medications.last;
          final medId = int.tryParse(newMed.id);
          if (medId != null) {
            await _createRappelsForMedication(
              rappelProvider: rappelProvider,
              medId: medId, name: newMed.name, schedules: newMed.schedules,
            );
          }
        }
      }
  ...
  Future<void> _deleteMedication() async {
    ...
    final medId = int.tryParse(widget.id!);
    if (medId != null) {
      await context.read<RappelProvider>().deleteRappelByMedicamentId(medId);
    }

    final success = await context.read<MedicationProvider>().deleteMedication(widget.id!);
    if (!mounted) return;
